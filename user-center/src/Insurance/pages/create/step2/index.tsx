...
class Inform extends Component<IProps, IState> {
  constructor (props) {
    super(props);
    this.state = {};
  }
  ...
  select (questionIndex, answerIndex) {
    ...
    if (answerType === 2) {
      if (answers[answerIndex].disSelectAll) {
        ...
        // 判断其他的选项是否有反选
        let dIndex;
        const disSelectAllArr = answers.filter((aItem, aIndex) => {
          aItem.disSelectAll === true && (dIndex = aIndex);
          return aItem.disSelectAll === true;
        });
        disSelectAllArr.length && (answers[dIndex].select = false);
      }
      answers[answerIndex].select = !answers[answerIndex].select;
    }
  ...
  watchData () {
    let num = 0;
    this.props.insurance.questionnaireList.forEach((item, index) => {
      const selectAnswer = item.answers.filter((aItem, aIndex) => {
